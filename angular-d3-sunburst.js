    ...
    return {
      ...
      controller: function ($scope, $element) {
        ...
        $scope.zoom = function (root, p) { // Zoom to the specified new root.
          ...
          var newData = $scope.partition.nodes(root).slice(1)

          $scope.path = $scope.path.data(newData, function (d) { return d.key })
          ...
          $scope.texts = $scope.texts.data(newData, function (d) { return d.key })
          $scope.texts.exit().remove()
          $scope.texts.enter().append('text')
          $scope.texts
            .style('opacity', 0)
        ...
        $scope.refresh = function () {
          ...
          var partitionedData = $scope.partition.nodes($scope.sunburst).slice(1)
